...
function createUploader(elid){            
    var uploader = new qq.FileUploader({
        ...
        onComplete: function(id, fileName, responseJSON){filesave(elid,fileName);}
        /*onCancel: function(id, fileName){console.log('onCancel');},*/
    });           
}
function filesave(id,fileName)
{
    var idoption = id.substr(14,id.length-14);   
    jQuery("#" + id).html("");
    jQuery("#" + id).html(fileName);
    image_upload.push("/sites/" + domainURL + "/files/images/questionnaire/" + fileName + '#' + idoption);  
}
/****************************************************************************************************/
...
function gridHeaderQuestion()
{
	...
	grid += "<div class='colheaderindice'>Indice</div>";
	grid += "<div class='colheaderquestion'>Question (drag to change order)</div>";
	...
	grid += "<div class='colheaderidquestion'>DB id</div>";
    grid += "<div class='colheaderoptions'>Options</div>";
	grid += "</div>";
        ...
        success: function(data){
    		...
    		jQuery.each(json, function(index, array) {
                ...
                if(index < json.length - 1) {
                    ...
                    divauxiliar += "</div>";
                    divauxiliar += "<div class='col colidquestion'>" + array['id_question'] + "</div></a>";
	...
	jQuery.ajax({
        ...
        success: function(data){
    		...
    		jQuery.each(json, function(index, array) {
                ...
                if (array['has_last_option'] == 1) {
                    jQuery("#cklastoptionquestion").attr('checked', true);
                } 
                else{
                    jQuery("#cklastoptionquestion").attr('checked', false);
                }
                ...
                if (array['subtype'] == 'multiple' || array['type']== 'sortable' || array['type']== 'dropdown' || array['type']== 'radiobutton'){
                    jQuery.ajax({
                        ...
                        success: function(data){
                            json = jQuery.parseJSON(data);
                    		jQuery.each(json, function(index, array) {
                  				  addmultipleoption(array['text'],array['image_path'],array['order'],array['id_option']);
                    		});
                            ...
                            .sortable({
                                stop: function (event,ui) {
                                    ...
                                    jQuery("#multipleoption tbody tr").each(function(x,elem){
                                        option.push(jQuery(elem).children().eq(1).html()); 
                                    });
                                }
                            });
                        }
                ...
                if (array['is_none'] == 1) {
                    jQuery("#ddfieldnoneoftheabove").attr('checked', true);
                } 
                else{
                    jQuery("#ddfieldnoneoftheabove").attr('checked', false);
                }  
                if (array['type']== 'grid'){
                    jQuery.ajax({
                        type: 'POST',
                    ...
                    success: function(data){
                        ...
                        {                  
                            ...
                            jQuery.each(json, function(index, array) {
                                  ...
                                  tr += "<td style='text-align:right;padding:3px 0px 3px 0px !important;'><img class='btn_delete_conditional_rule' src='"+base_url+"/images/boton_quitar.png' width='25' style='margin-right:1px; vertical-align: middle; cursor:pointer;' border='0' /></td>";
                                  tr += "</tr>";
...
function saveQuestion()
{
	...
	if (jQuery("#txtfieldquestion").val() == "" || jQuery("#txtfieldquestion").val() == "Please type here" || jQuery("#ddfieldtype").val() == "")
    {
       alert('Please, fill all options.');
    }
    else
    {
    	if(jQuery("#ddfieldtype").val() == "radiobutton" || jQuery("#ddfieldtype").val() == "sortable" || jQuery("#ddfieldtype").val() == "sortable"){
    	   if (option.length > 0){
    	       grabar = true;
               error_mensaje = "";
    	   }
               ...
               grabar = false;
               error_mensaje = "Please, fill any option."
           }           
    	}
        else{grabar = true;}
        if(jQuery("#ddfieldtype").val() == "checkbox" && jQuery("#ddfieldsubtype").val() == "multiple"){
           if (option.length > 0){
    	       grabar = true;
               error_mensaje = "";
    	   }
        ...
        if (grabar == true){
            ...
            objregistro.none_above      = 0; //Null by defect
        	...
        	if (objregistro.subtype == 'multiple')
        	{
        		objregistro.option = option;
                objregistro.random_options = jQuery("#ddfieldrandomoptions").attr('checked') == true ? 1 : 0;
                objregistro.none_above = jQuery("#ddfieldnoneoftheabove").attr('checked') == true ? 1 : 0;	
        	}
...
function ModalPopupBodyQuestion()
{
    ...
    tbody +="</td>";
    tbody += "<td colspan='1'><input type='checkbox' id='cklastoptionquestion' /><span>Last option</span></td>";
    tbody += "<td colspan='2'>";
    ...
    tbody += "<div class='divGeneral' style='padding-top:10px; background-color:#EF4619;  padding-bottom:10px'><a href='#'>Conditional rules</a></div>";
...
function subtypeQuestion(){
    ...
    switch(jQuery("#ddfieldsubtype").val())
	{
		...
		case 'multiple': 
            ...
            if (jQuery("#ddfieldtype").val() == 'checkbox'){
                jQuery("#ddfieldnoneoftheabove").removeAttr("disabled");
            }
		    break;
		case 'radiobuttons': 
			jQuery("#gridtrhead").removeClass("gridtr");
        ...
        case '':
            ...
            {                
                ...
                jQuery("#ddfieldrandomoptions").removeAttr("checked");
                jQuery("#multipletrbuttonadd").addClass("multipletrbuttonadd");
                jQuery("#ddfieldnoneoftheabove").attr("disabled","disabled"); 
            }
            break;
		default: 
		    jQuery("#textmultiple").val("");
...
function addmultipleoption(val,image_path,order,id_option)
{
    var value;
    ...
    if (!val){
       ...
       { 
		   ...
		   row += "<td width='5%'>"+id_option+"</td>";
           row += "<td width='60%'>"+value+"</td>";
           row += "<td id='file-uploader-" + option.length + "' width='30%'></td>";
           row += "<td width='5%'><a class='deleteOption'><img src='"+base_url+"/images/boton_quitar.png' width='25'  border='0' align='right' /></a></td></tr>";
           table.append(row);	       
       ...
       else
       {
        ...
        if (image_path != null){
          ...
          image_upload.push("/sites/" + domainURL + "/files/images/questionnaire/" + fileName + '#' + order);  
        }
        else{
            fileName = "&nbsp;";
        }
    ...
    if (!val){
       if (jQuery("#textgridrow").val() != ''){
        ...
        if (image_pathROW != null){
          ...
          image_uploadROW.push("/sites/" + domainURL + "/files/images/questionnaire/" + fileName + '#' + order);  
        }
        value = val;
        printGRIDoptionROW(fileName,value);
    }
...
function addgridoptioncol(val,image_pathCOL,order)
{
    ...
    if (!val){
        ...
        if (image_pathCOL != null){
          ...
          image_uploadCOL.push("/sites/" + domainURL + "/files/images/questionnaire/" + fileName + '#' + order);  
        }
        value = val;
        printGRIDoptionCOL(fileName,value)
    }
...
function filesave(id,fileName,type)
{
    switch(type){
        ...
        case 'row':
            ...
            image_uploadROW.push("/sites/" + domainURL + "/files/images/questionnaire/" + fileName + '#' + idrow);   
        break;
        case 'col':
            var idcol = id.substr(17,id.length-17);
            image_uploadCOL.push("/sites/" + domainURL + "/files/images/questionnaire/" + fileName + '#' + idcol);
        break;
    }
    jQuery("#" + id).html("");
    jQuery("#" + id).html(fileName);    
